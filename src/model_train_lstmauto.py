...
# ==================== Model Selection ====================
# Original model (4-layer LSTM with ReLU activation):
from models.LSTM_Autoenc import *
from utils.config_LSTM_Autoenc import *

# Keras-reproduced model (3-layer LSTM with tanh activation, matching the notebook structure):
# from models.LSTM_Autoenc_keras import LSTM_Autoencoder_Keras as LSTM_Autoencoder
# from utils.config_keras import config
# ================================================
...
def patient_level_bootstrap_auroc(prob_list, label_list, patient_ids, n_bootstrap=1000):
    """
    Aggregate prediction-window outputs to patient level (mean probability),
    then bootstrap at the patient level to compute 95% CI for AUROC.
    Returns (point_auroc, ci_low, ci_high).
    """
    prob_arr   = np.array(prob_list)
    label_arr  = np.array(label_list)
    patient_arr = np.array(patient_ids)

    unique_pats = np.unique(patient_arr)
    pat_probs  = np.array([prob_arr[patient_arr == p].mean() for p in unique_pats])
    pat_labels = np.array([int(label_arr[patient_arr == p][0]) for p in unique_pats])

    point_auroc = roc_auc_score(pat_labels, pat_probs)

    auroc_boots = []
    n_pats = len(unique_pats)
    rng = np.random.default_rng(seed=42)
    for _ in range(n_bootstrap):
        idx = rng.choice(n_pats, size=n_pats, replace=True)
        if len(np.unique(pat_labels[idx])) < 2:
            continue
        auroc_boots.append(roc_auc_score(pat_labels[idx], pat_probs[idx]))

    ci_low  = np.percentile(auroc_boots, 2.5)
    ci_high = np.percentile(auroc_boots, 97.5)
    return point_auroc, ci_low, ci_high


class Trainer:
    def __init__(self, model, loaders, optimizer, scheduler, lossfn, rec_lossfn, params, test_patids=None):
        ...
        self.params = params
        self.test_patids = test_patids  # patient IDs aligned with test set (patient split only)
        self.epochs_run = 0
    ...
    def train(self):
        ...
        for epoch in range(self.epochs_run, self.params.num_epochs):
            ...
            test_acc, test_f, test_auroc, test_auprc, test_pre, test_rec, _, _ = self.test_epoch(test_dataloader, self.model, self.gpu_id, self.params.thres)
            ...
            print(
                ...
                "Test Acc:{:.3f}, Test F1-score:{:.3f}, Test AUROC:{:.3f}, Test AUPRC:{:.3f}, Test Precision:{:.3f}, Test Recall:{:.3f}!!!\n".format(
                    epoch, train_loss, val_loss,
                    train_acc, train_f, train_auroc, train_auprc, train_pre, train_rec,
                    val_acc, val_f, val_auroc, val_auprc, val_pre, val_rec,
                    test_acc, test_f, test_auroc, test_auprc, test_pre, test_rec))
            if best_f1 < val_f:
                nobetter = 0
                best_f1 = val_f
    ...
    def test_epoch(self, test_dataloader, model, gpu_id, threshold=0.5):
        model.eval()
        pred_list, prob_list, label_list = [], [], [] 
        with torch.no_grad():
        ...
        return test_acc, test_f, test_auroc, test_auprc, test_pre, test_rec, prob_list, label_list

    def test(self, test_dataloader, model, gpu_id, threshold=0.5):
        test_acc, test_f, test_auroc, test_auprc, test_pre, test_rec, prob_list, label_list = self.test_epoch(test_dataloader, model, gpu_id, threshold)

        n_boot = getattr(self.params, 'n_bootstrap', 1000)

        if self.test_patids is not None:
            pt_auroc, ci_low, ci_high = patient_level_bootstrap_auroc(
                prob_list, label_list, self.test_patids, n_bootstrap=n_boot)
            print("Patient-level performance on test data — "
                  f"Acc:{test_acc:.3f}, F1:{test_f:.3f}, AUROC:{pt_auroc:.3f} "
                  f"(95% CI: {ci_low:.3f}–{ci_high:.3f}), AUPRC:{test_auprc:.3f}, "
                  f"Precision:{test_pre:.3f}, Recall:{test_rec:.3f} "
                  f"[{n_boot} bootstrap iterations, {len(np.unique(self.test_patids))} patients]")
...
def get_dataset(data_sources, month, seed, train_ratio=0.7, test_ratio=0.2,
    ...
    test_patids = patient_of_subseq[test_mask]
    ...
    return train_dataset, valid_dataset, test_dataset, tf_dim, fea_dim, test_patids

def get_dataloader(params, train_ratio=0.7, test_ratio=0.2):
    data_path   = getattr(params, 'data_path',
                          str(STEP1_3D_NPZ))
    patid_path  = getattr(params, 'patid_path',
                          str(STEP1_SUBSEQ_NPZ))

    train_dataset, valid_dataset, test_dataset, tf_dim, fea_dim, test_patids = get_dataset(
        params.data_sources, params.month, params.seed,
        train_ratio=train_ratio, test_ratio=test_ratio,
        data_path=data_path, patid_path=patid_path)
    ...
    return [train_loader, valid_loader, test_loader], tf_dim, fea_dim, test_patids
...
def load_model_objs(tf_dim, fea_dim, params):
    ...
    model = LSTM_Autoencoder(tf_dim, fea_dim, layers, dp_rate)
    model = model.to(device)
    
    # weight_decay plays the role of L2 regularization
...
def main():
    ...
    loaders, tf_dim, fea_dim, test_patids = get_dataloader(params)
    data_time2 = datetime.datetime.now()
    print(f'Data Loading Time is {(data_time2 - data_time1).seconds}s. ')
    split_info = params.split_level
    print(f'Model name: {params.model_name}, split: {split_info}, data sources: {params.data_sources}, month: {params.month}, batch: {params.batchSize}, epoch:{params.num_epochs}, lr: {params.lr}, layers: {params.layers}, cls_weight: {params.cls_weight}, rec_weight:{params.rec_weight}, drop: {params.dropout}, rec_drop: {params.rec_dropout}')
    train_time1 = datetime.datetime.now()
    model, optimizer, scheduler, lossfn, rec_lossfn = load_model_objs(tf_dim, fea_dim, params)
    trainer = Trainer(model, loaders, optimizer, scheduler, lossfn, rec_lossfn, params, test_patids=test_patids)
    trainer.train()
